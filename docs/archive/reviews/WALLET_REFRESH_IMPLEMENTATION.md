...
-   **Code:** Refactored `apps/web/src/WalletView.tsx` to use semantic `nostrstack-dialog` classes.
-   **Cleanup:** Deleted `apps/web/src/styles/withdraw.css`.

### 2. Profile View
-   **Visuals:** Updated the "Tip" and "Lightning Address" cards to use the new design tokens (lighter backgrounds, cleaner borders).
-   **Code:** Consolidated styles into `web.css` under "Profile Payment Cards" section.
-   **Cleanup:** Deleted `lightning-card.css` and `profile-tip.css`.

### 3. Architecture
-   **Single Source of Truth:** All web styles now live in `apps/web/src/web.css`, which is mapped to the `packages/embed/src/styles.ts` tokens. This ensures that changing a theme variable (like primary color) updates the entire app, including the embedded widgets.

## Known Issues (Environment)
-   **FeedView Crash:** The automated tests for `web-flow.spec.ts` and `withdraw.spec.ts` are failing because `FeedView` crashes upon mount. This appears to be an environmental issue with the `mock-relay` WebSocket connection or the `useFeed` hook in the test environment.
