...
- Use `make dev` (or `pnpm dev:logs`, wrapper for `scripts/dev/logs.sh`) when developing. It starts API + web with prefixed, timestamped output and writes to `.logs/dev/api.log` and `.logs/dev/web.log`.
...
- Keep a terminal pane tailing logs: `tail -f .logs/dev/api.log .logs/dev/web.log`.
- Include log snippets in issue notes when you hit errors.
- Telemetry websocket: the web app connects via the current page origin (`wss://<host>/ws/telemetry` in dev). Run `make dev` so the Vite proxy forwards to the API.

## UI changes must be confirmed in MCP Chrome
- Launch Chrome DevTools MCP bridge: `./scripts/mcp-devtools-server.sh` (server) and `./scripts/mcp-chrome.sh` (client) while the web app is running.
...
2) Inspect API + web logs for errors and note any findings in the issue.
