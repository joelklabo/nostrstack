...
## Runbook
1) **Rotate DB password** (if needed): `az postgres flexible-server update -g satoshis-stg-west-rg -n nostrstack-pg-west --admin-password '<new>'`, then update KV `lnbits-prod-database-url` + `database-url-prod` and redeploy with a new `LNBITS_REV` stamp.
2) **Swap LND to mainnet**:
   - Update KV secrets `lnd-mainnet-endpoint`, `lnd-mainnet-macaroon-hex`, `lnd-mainnet-tls` with mainnet values (preflight will verify presence). The helper `./scripts/voltage-setup.sh` guides Voltage node selection and KV updates.
   ...
   - Check logs: `az containerapp logs show -n lnbits-prod-west -g satoshis-stg-west-rg --type console --tail 100`
