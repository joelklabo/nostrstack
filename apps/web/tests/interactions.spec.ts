...
async function ensureFeedHasPost(page: Page) {
  const existingPost = page.locator('[data-testid="web-event-card"]').first();
...
test.describe('App Interactions', () => {
  ...
  test('toggle view source in feed', async ({ page }) => {
    ...
    const postCard = page.locator('[data-testid="web-event-card"]').first();
    ...
    const sourceView = postCard.locator('[data-testid="web-event-json"]');
