...
function fallbackSessionUrls(): { webUrl: string | undefined; apiUrl: string | undefined } {
  const portEnv = process.env.DEV_SERVER_PORT;
  const webPort = process.env.WEB_PORT ?? portEnv ?? '4173';
  const apiPort = process.env.API_PORT ?? portEnv ?? '3001';
  const protocol = process.env.QA_USE_HTTPS !== '0' ? 'https' : 'http';

  return {
    webUrl: `${protocol}://localhost:${webPort}`,
...
async function detectManagedSessionUrls(): Promise<ManagedSession> {
  ...
  try {
    ...
    return { webUrl: undefined, apiUrl: undefined };
  }

  const sessionFiles = files.filter((file) => file.endsWith('.session'));
  if (!sessionFiles.length) return { webUrl: undefined, apiUrl: undefined };
  ...
  const sessions = await Promise.all(
    sessionFiles.map(async (fileName) => {
      ...
      return {
        ...
        webPort: parseSessionValue(contents, 'NOSTRDEV_SESSION_WEB_PORT')
      };
    })
  );
  ...
  const preferred = sessions
    .filter(
      (session) => session.webPort && (!requestedAgent || session.agent === requestedAgent)
    )
    .sort((a, b) => b.mtimeMs - a.mtimeMs);
  const fallback = sessions
    .filter((session) => session.webPort)
    .sort((a, b) => b.mtimeMs - a.mtimeMs);

  for (const session of [...preferred, ...fallback]) {
    const webPort = Number(session.webPort);
    const apiPort = Number(session.apiPort);
    if (!Number.isFinite(webPort) || webPort <= 0) continue;

    const webCandidates = [`http://localhost:${webPort}`, `https://localhost:${webPort}`];
  ...
  return { webUrl: fallbackUrls.webUrl, apiUrl: fallbackUrls.apiUrl };
}

async function ensureWebAvailable(baseUrl: string) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    try {
      ...
      if (attempt === maxAttempts) {
        const localHint = `Web server not reachable at ${baseUrl}. Start dev servers with "pnpm dev:logs" and try again, or set WEB_URL to a running instance.`;
        const remoteHint = `Web server not reachable at ${baseUrl}. Check the URL or set WEB_URL to a running instance.`;
...
async function main() {
  ...
  const defaultPort = devServerPort ?? process.env.WEB_PORT ?? '4173';
  const protocol = useHttps ? 'https' : 'http';
  const fallbackUrl = `${protocol}://localhost:${defaultPort}`;
  const baseUrl = process.env.WEB_URL || detected.webUrl || fallbackUrl;
  const _apiUrl =
    process.env.API_URL ?? detected.apiUrl ?? `${protocol}://localhost:${devServerPort ?? '3001'}`;

  if (detected.webUrl || detected.apiUrl) {
    const parts: string[] = [];
    if (detected.webUrl) parts.push(`web=${detected.webUrl}`);
    if (detected.apiUrl) parts.push(`api=${detected.apiUrl}`);
    console.log(`ℹ️ using managed dev session: ${parts.join(', ')}`);
  }

  const hasExplicitUrl =
    process.env.WEB_URL || process.env.DEV_SERVER_PORT;
  if (!hasExplicitUrl && !detected.webUrl && !process.env.NOSTRDEV_AGENT) {
    console.log(
      `ℹ️ No managed session detected. Using defaults (web=${fallbackUrl}). Set WEB_URL or API_URL, or run under pnpm dev:logs for managed sessions.`
    );
  }

  if (process.env.NOSTRDEV_AGENT && !detected.webUrl && !detected.apiUrl && !hasExplicitUrl) {
    console.log(
      `ℹ️ No managed session for agent '${process.env.NOSTRDEV_AGENT}'. Using fallback URLs. Set WEB_URL or API_URL, or run under pnpm dev:logs for managed sessions.`
    );
  }
  ...
  await ensureWebAvailable(baseUrl);
