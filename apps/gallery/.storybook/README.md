# Storybook for NostrStack Gallery

This directory contains Storybook configuration and stories for the NostrStack Gallery UI components.
...
- **Payment Components**: PaymentModal, ZapButton, SendSats (from `@nostrstack/blog-kit`)
- **Nostr Components**: PostEditor, ReplyModal, ReactionButton (from `@nostrstack/blog-kit`)
...
The static build is output to `apps/gallery/storybook-static/`.
...
```typescript
import { AuthProvider } from '@nostrstack/blog-kit/auth';
import { NostrstackConfigProvider } from '@nostrstack/blog-kit/context';
