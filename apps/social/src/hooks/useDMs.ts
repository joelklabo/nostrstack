...
export function useDMs() {
  ...
  const sendDM = useCallback(
    async (peerPubkey: string, text: string) => {
      ...
      try {
        ...
        // Continue even if all relays fail - message will be sent on next attempt
        console.warn('[DMs] Failed to publish to all relays, message may not arrive');
      }
